...
if (inputDist) {
    ...
    if (rota) {
        ...
        if (estado.escolaProximaUser === false && (estado.distancia >= 1500 || excecaoGarantida) && estado.encaminhamentoOk === null) {
            ...
            // BUSCA DE ENCAMINHAMENTOS BLINDADA E DESVINCULADA DA OUTRA IA
            let msgEncaminhamentoHtml = "";
            const docFinalBusca = window.getAlvoDocument();
            let raAlunoRaw = "";
            
            // 1. Tenta capturar o RA direto do input HTML escondido
            const inputRaProdesp = docFinalBusca.getElementById('ra_prodesp_search') || document.getElementById('ra_prodesp_search');
            if (inputRaProdesp) {
                raAlunoRaw = inputRaProdesp.value || inputRaProdesp.innerText || '';
            }
            
            // 2. Se falhar, tenta pegar do SharedStore
            if (!raAlunoRaw) {
                raAlunoRaw = window.getSharedStoreValue?.('raAluno') || window.getSharedStoreValue?.('ra_prodesp_search') || '';
            }
            
            // 3. Limpeza severa do RA (mantém apenas números, ignora traços ou letras)
            const raAluno = raAlunoRaw.replace(/\D/g, '');

            // 4. Captura a variável global de encaminhamentos independentemente de como ela foi salva
            let dbEncaminhamentos = null;
            if (typeof encaminhamentos !== 'undefined') dbEncaminhamentos = encaminhamentos;
            else if (typeof window.encaminhamentos !== 'undefined') dbEncaminhamentos = window.encaminhamentos;
            else if (typeof window.encaminhamentosDB !== 'undefined') dbEncaminhamentos = window.encaminhamentosDB;
            else if (typeof window.encaminhamentosData !== 'undefined') dbEncaminhamentos = window.encaminhamentosData;
            else if (typeof HISTORICO_MASTER !== 'undefined') dbEncaminhamentos = HISTORICO_MASTER.encaminhamentos;

            if (raAluno && dbEncaminhamentos) {
                // Tenta encontrar a chave exata
                let listaEncaminhamentos = dbEncaminhamentos[raAluno];
                
                // Se falhar e o RA tiver mais de 5 dígitos (ex: RA 1162290335 com dígito embutido, mas no banco é 116229033), tenta sem o último dígito
                if (!listaEncaminhamentos && raAluno.length > 5) {
                    const raSemDigito = raAluno.slice(0, -1);
                    listaEncaminhamentos = dbEncaminhamentos[raSemDigito];
                }

                if (Array.isArray(listaEncaminhamentos) && listaEncaminhamentos.length > 0) {
                    const maisRecente = listaEncaminhamentos.reduce((prev, current) => (prev.ano > current.ano) ? prev : current);
                    ...
                    console.log(`[ASSISTENTE] RA ${raAluno} não localizado no banco de dados importado.`);
                }
            } else {
                console.error("[ASSISTENTE] Falha técnica ao pesquisar encaminhamentos. Detalhes:", {
                    "RA Capturado": raAluno || "NENHUM RA ENCONTRADO NO DOM (Input #ra_prodesp_search ausente)",
                    "Banco Carregado?": dbEncaminhamentos ? "SIM" : "NÃO (O arquivo .js de encaminhamentos não foi importado no HTML principal)"
                });
            }
